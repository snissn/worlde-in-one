...
test("preview metadata includes the shared challenge code", async () => {
  ...
  const meta = previewMetaForUrl("https://example.com/?seed=AbC-234!");
  ...
  assert.equal(meta.url, "https://example.com/?seed=abc234");
  ...
  assert.match(preview, /<meta property="og:url" content="https:\/\/example\.com\/\?seed=abc234" \/>/);
  assert.match(preview, /<meta name="twitter:title" content="Wordle in One Challenge ABC234" \/>/);
  assert.match(preview, /<link rel="canonical" href="https:\/\/example\.com\/\?seed=abc234" \/>/);
});
